...
public class ACO {
	...
	public void run(int maxgen){
		for(int runTime=0; runTime<maxgen; runTime++){
			...
			for(int i=0; i<antcount; i++){
				...
				System.out.println("ant "+i+" distance: "+ants.get(i).tourLength);
				ants.get(i).CalDelta();
				if(ants.get(i).tourLength<bestLength){  
					//保留最优路径  
	                bestLength = ants.get(i).tourLength;  
	                System.out.println("generation "+runTime+" ant "+i+" find new solution: "+bestLength);
	                for(int j=0;j<tasks;j++){  
    ...
    public void ReportResult(){  
        ...
        for(int j=0; j<tasks; j++)
        {
        	System.out.println(bestTour[j].task+"is alocated to: "+bestTour[j].vm);
        }
